...
const plants = [{
  ...
  test('rendring the ShoppingCart whithout errors', ()=>{
    render(<ShoppingCart cart={plants} />)
  })

  test('displayig plants names in the ShoppingCart component', ()=>{
    render(<ShoppingCart cart={plants} />)
    const peperomia = screen.getByText(/Peperomia Rosso/i)
    expect(peperomia).toBeInTheDocument()

    const string = screen.getByText(/String of Dolphins/i)
    expect(string).toBeInTheDocument()
  })

  test('displaying plants prices in the ShoppingCart component', ()=>{
      render(<ShoppingCart cart={plants} />)
  ...
  test('displaying plants images in ShoppingCart component', ()=>{
    render(<ShoppingCart cart={plants} />)

    const img1 = screen.getByAltText(/Peperomia Rosso/i)
    expect(img1).toBeInTheDocument()

    const img2 = screen.getByAltText(/String of Dolphins/i)
